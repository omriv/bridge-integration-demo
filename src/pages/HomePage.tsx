...
export function HomePage() {
  const { customer, customers, currentCustomerId, wallets, loading, error, loadCustomerData, loadCustomers, setCurrentCustomerId, refreshAll, virtualAccounts, virtualAccountsLoading, externalAccounts } = useData();
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsSearching(true);
    try {
      await loadCustomers(searchEmail);
    } finally {
              ...
              <div className="relative w-full">
                ...
                {isDropdownOpen && (
                  <div className="absolute z-20 w-full mt-2 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg shadow-xl max-h-96 overflow-y-auto">
                    {/* Search Field */}
                    <div className="p-3 border-b border-neutral-200 dark:border-neutral-700 sticky top-0 bg-white dark:bg-neutral-800 z-10">
                      <form onSubmit={handleSearch} className="relative">
                        <input
                          ...
                          className="w-full pl-9 pr-3 py-2 bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 dark:text-white"
                        />
                        ...
                        <button 
                          type="submit"
                          className="absolute right-2 top-1.5 px-2 py-1 bg-amber-500/10 text-amber-600 dark:text-amber-400 rounded text-xs font-medium hover:bg-amber-500/20 transition-colors"
                        >
                          Search
                        </button>
                      </form>
                    ...
                    {isSearching ? (
                      <div className="flex items-center justify-center p-8">
                        <div className="text-center">
                          ...
                          <p className="text-neutral-500 dark:text-neutral-400 text-sm">Searching...</p>
                        </div>
                      </div>
                    ) : (
                      customers.map((cust) => (
