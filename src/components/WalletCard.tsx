...
export function WalletCard({ wallet, virtualAccounts = [] }: WalletCardProps) {
  ...
  const getChainColor = (chain: string) => {
    const colors: Record<string, string> = {
      ethereum: 'bg-teal-500/10 text-teal-400 border-teal-500/30',
      ...
      optimism: 'bg-red-500/10 text-red-400 border-red-500/30',
      solana: 'bg-fuchsia-500/10 text-fuchsia-400 border-fuchsia-500/30',
      avalanche: 'bg-rose-500/10 text-rose-400 border-rose-500/30',
      bsc: 'bg-yellow-500/10 text-yellow-400 border-yellow-500/30',
    };
    return colors[chain.toLowerCase()] || 'bg-slate-700 text-slate-400 border-slate-600';
  };

  const getCurrencyColor = (currency: string) => {
    const colors: Record<string, string> = {
      usdc: 'bg-blue-500/10 border-blue-500/30 text-blue-400',
      usdt: 'bg-green-500/10 border-green-500/30 text-green-400',
      usdb: 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400',
      dai: 'bg-amber-500/10 border-amber-500/30 text-amber-400',
      eth: 'bg-slate-700 border-slate-600 text-slate-300',
      weth: 'bg-slate-700 border-slate-600 text-slate-300',
      btc: 'bg-orange-500/10 border-orange-500/30 text-orange-400',
      wbtc: 'bg-orange-500/10 border-orange-500/30 text-orange-400',
      matic: 'bg-purple-500/10 border-purple-500/30 text-purple-400',
      sol: 'bg-violet-500/10 border-violet-500/30 text-violet-400',
      avax: 'bg-rose-500/10 border-rose-500/30 text-rose-400',
      bnb: 'bg-yellow-500/10 border-yellow-500/30 text-yellow-400',
    };
    return colors[currency.toLowerCase()] || 'bg-slate-700 border-slate-600 text-slate-300';
  };
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <div className="space-y-1.5">
              {/* Wallet ID */}
              <div>
                <p className="text-xs text-slate-500 mb-0.5">Wallet ID</p>
                <div className="flex items-center gap-1">
                  <p className="font-mono text-xs text-white break-all flex-1">{wallet.id}</p>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
              ...
              <div>
                ...
                <div className="flex items-center gap-1">
                  <p className="font-mono text-xs text-white break-all flex-1">{wallet.address}</p>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
              ...
              <div className="mt-2 p-2 bg-green-500/10 border border-green-500/30 rounded">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-green-400">💵 Total Value</span>
              ...
              <div className="mt-2 flex gap-2">
                <button
                  ...
                  className="flex-1 bg-blue-600 text-white px-3 py-1.5 rounded text-xs font-semibold hover:bg-blue-500 transition-all shadow-sm"
                >
                  📋 Overview
                </button>
                <button
                  ...
                  className="flex-1 bg-green-600 text-white px-3 py-1.5 rounded text-xs font-semibold hover:bg-green-500 transition-all shadow-sm"
                >
                  💸 Create Transfer
                </button>
              </div>
          ...
          <div className="ml-4">
            <svg
              className={`w-6 h-6 text-slate-500 transition-transform ${isExpanded ? 'rotate-180' : ''}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
